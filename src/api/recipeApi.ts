...
export const recipeApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getCategories: builder.query<Category[], void>({
            ...
            transformResponse: (response: ListResponse<Category>) => response.meals || [],
        }),
        getIngredients: builder.query<Ingredient[], void>({
            query: () => 'list.php?i=list',
            transformResponse: (response: ListResponse<Ingredient>) => response.meals || [],
        }),
        getRecipesByArea: builder.query<Recipe[], string>({
            query: (area) => `filter.php?a=${area}`,
            transformResponse: (response: ListResponse<Recipe>) => response.meals || [],
        }),
        ...
        lookupRecipe: builder.query<Recipe | null, string>({
            ...
            transformResponse: (response: ListResponse<Recipe>) => (response.meals && response.meals.length > 0 ? response.meals[0] : null),
        }),
    }),
});
