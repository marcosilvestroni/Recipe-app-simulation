...
describe('PreferenceForm', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        useGetAreasQueryMock.mockReturnValue({ data: [{ strArea: 'Italian' }, { strArea: 'French' }] });
        useGetCategoriesQueryMock.mockReturnValue({ data: [{ strCategory: 'Beef' }, { strCategory: 'Vegan' }] });
        useGetIngredientsQueryMock.mockReturnValue({ data: [{ strIngredient: 'Chicken' }, { strIngredient: 'Garlic' }] });
    });

    it('renders step 1 with areas', () => {
        render(<PreferenceForm {...mockProps} />);
        expect(screen.getByText('Where would you like to eat?')).toBeInTheDocument();
        expect(screen.getByText('Select a cuisine style or region.')).toBeInTheDocument();
    ...
    it('enables next button and calls onNext in step 1 when area selected', () => {
        const propsWithArea = { ...mockProps, preferences: { ...mockProps.preferences, area: 'Italian' } };
    ...
    it('handles category selection in step 2', () => {
        ...
        expect(mockProps.onUpdate).toHaveBeenCalledWith({ categoryOrIngredient: 'Beef' });
    });

    it('switches to ingredient strategy', () => {
        render(<PreferenceForm {...mockProps} step={2} />);
        fireEvent.click(screen.getByLabelText('Main Ingredient'));
        expect(mockProps.onUpdate).toHaveBeenCalledWith({ strategy: 'ingredient', categoryOrIngredient: '' });
    });
    ...
    it('renders ingredient search when strategy is ingredient', () => {
        const propsIngredient = { 
            ...mockProps, 
            step: 2, 
            preferences: { ...mockProps.preferences, strategy: 'ingredient' as const } 
        };
    ...
    it('filters and selects ingredients', () => {
        ...
        expect(mockProps.onUpdate).toHaveBeenCalledWith({ categoryOrIngredient: 'Chicken' });
    });
    ...
    it('updates local input state when props change to a new value', () => {
         const propsIngredient = { 
            ...mockProps, 
            step: 2, 
            preferences: { ...mockProps.preferences, strategy: 'ingredient' as const, categoryOrIngredient: 'Garlic' } 
        };
        ...
        // Update prop
        rerender(<PreferenceForm {...{...propsIngredient, preferences: { ...propsIngredient.preferences, categoryOrIngredient: 'Chicken' }}} />);
